...
int main(int argc, char* argv[]) {
	...
	if (argc < 3) {
		printf("Please enter values for both ndim and nblas \n");
		exit(1);
	}
	if (argc > 3) {
		printf("Please enter ONLY values for ndim and nblas \n");
		exit(1);
	}

	int ndim  = atoi(argv[1]);
	int nblas = atoi(argv[2]);
...
//	print_double_complex_matrix(A, ndim);
	...
	// CAN WE CALL CUBLAS ALL ON THE SAME MATRIX? TRY CALLING IT nblas TIMES	
	...
	// WE NOW NEED nblas x THE MEMORY ALLOCATED FOR RESULTS
	...
	// Kernel launch configuration - we will call cublas once for each sample
	// We will always sample in even multiples of 1024 



	int nblks = nblas / 1024; 
	

	dim3 grid = dim3(nblks, 1, 1);
	dim3 block = dim3(1024, 1, 1);

        call_zgemv_gpu<<<grid, block>>>(m, n, &d_scalar_params->alpha, A_gpu, ndim, vec_gpu, &d_scalar_params->beta, result_gpu);
	...
	for (int i = 0; i < 10; i++) {
		print_double_complex_vector(&result[i * ndim], ndim);
	}
