...
t_window = 0.15
rholms_intp, cross_termsU, cross_termsV, rholms = factored_likelihood.precompute_likelihood_terms(fiducial_epoch, t_window, P, data_dict, psd_dict, opts.l_max, fmax, False, inv_spec_trunc_Q, T_spec)
...
if opts.pin_to_sim:
    ...
    logL = factored_likelihood.factored_log_likelihood(P, rholms_intp, cross_termsU, opts.l_max)
    ...
    for t in tvals:
        ...
        logLs.append(factored_likelihood.factored_log_likelihood(P, rholms_intp, cross_termsU, opts.l_max))
    import matplotlib
    matplotlib.use("Agg")
...
if not opts.time_marginalization:
    ...
    def likelihood_function(right_ascension, declination, t_ref, phi_orb, inclination, psi, distance):
        ...
        for ph, th, tr, phr, ic, ps, di in zip(right_ascension, declination,
            ...
            lnL[i] = factored_likelihood.factored_log_likelihood(P, rholms_intp, cross_termsU, cross_termsV, opts.l_max)
            i+=1
